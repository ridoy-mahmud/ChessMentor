...
import { BOT_ORDER, BOTS } from "@/lib/chess/bots";
        ...
        <div className="mx-auto grid max-w-6xl gap-10 px-4 pt-14 pb-16 sm:px-6 md:grid-cols-[1.05fr_1fr] md:pt-20 md:pb-24">
          <div className="flex flex-col justify-center">
            ...
            <div className="mt-10 grid grid-cols-3 gap-4 text-sm">
              <Stat icon={BookOpen} label="Lessons" value={<CountUp target={8} />} />
              <Stat icon={Puzzle} label="Puzzles solved" value={<CountUp target={132} />} />
              <Stat icon={Zap} label="Streak" value={<CountUp target={5} suffix=" days" />} />
            </div>
          </div>
            ...
            {BOT_ORDER.map((id) => {
              ...
              return (
                <Link
                  ...
                  to="/play"
                  search={{ bot: id }}
                  className="group rounded-xl border bg-card p-4 text-center shadow-e1 transition-all hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-e2"
                >
                  <img
                    src={b.avatar}
                    alt={b.name}
                    ...
                    height={96}
                    loading="lazy"
                    className="mx-auto h-20 w-20 rounded-full object-cover ring-2 ring-transparent transition-all group-hover:ring-primary/30"
                  />
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {[
            ...
            },
            {
              ...
              body: "Board-vision drill — see a square, click it fast. Speed leaderboard.",
              href: "/coordinate" as const,
            },
            {
              icon: LineChart,
            ...
            <ScrollReveal key={c.title} delay={i * 80}>
              <Link
                to={c.href}
                className="group block h-full rounded-xl border bg-card p-6 shadow-e1 transition-all hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-e2"
              >
